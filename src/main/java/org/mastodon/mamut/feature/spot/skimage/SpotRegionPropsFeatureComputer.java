...
public class SpotRegionPropsFeatureComputer extends AbstractResettableFeatureComputer
{
	...
	public void createOutput()
	{
		if ( null == output )
		{
			...
			// Create a new output.
			output = new SpotRegionPropsFeature( map );
		}
	}
	...
	public void run()
	{
		...
		{
			trackAstraRegionProps.compute( bdvData.getSources(), model, output, status );
			addCoords();
		}
		catch ( Exception e )
		{
	...
	private void addCoords()
	{
		...
		{
			for ( int t = getMinTimepoint( model ); t <= getMaxTimepoint( model ); t++ )
			{
				AffineTransform3D transform = DetectionUtil.getTransform( sources, t, sourceId.get(), 0 );
