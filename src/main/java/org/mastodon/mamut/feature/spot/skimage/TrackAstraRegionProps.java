...
public class TrackAstraRegionProps extends ApposeProcess
{
	...
	protected TrackAstraRegionProps() throws IOException
	{
		super();
	}

	void compute( final List< SourceAndConverter< ? > > sources, final Model model, final SpotRegionPropsFeature feature,
			final FeatureComputationStatus status )
	{
		...
		for ( SourceAndConverter< ? > sourceAndConverter : sources )
		{
			...
			{
				computeSource( source, sourceIndex, timepoint, model, feature );
			...
			sourceIndex++;
			if ( sourceIndex == 1 )
				break; // TODO: only process the first source for testing
		}
	}
	...
	{
		int level = 0; // Use highest resolution level.
