...
import org.mastodon.mamut.util.GeometryUtils;
		...
		for ( BranchSpot root : RootFinder.getRoots( branchGraph ) )
		{
			...
			while ( depthFirstIterator.hasNext() )
			{
				...
				while ( it.hasNext() )
				{
					...
					if ( isPreviousCoordinatesSet )
						totalDistanceDuringCellLifeCycle +=
								GeometryUtils.getEuclideanDistance( currentSpotCoordinates, previousSpotCoordinates );
					else
						totalDistanceDuringCellLifeCycle +=
								GeometryUtils.getEuclideanDistance( currentSpotCoordinates, branchStartCoordinates );
				...
				double directDistanceDuringCellLifeCycle =
						GeometryUtils.getEuclideanDistance( branchStartCoordinates, currentSpotCoordinates );
