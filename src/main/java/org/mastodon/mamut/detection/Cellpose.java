...
public class Cellpose extends Segmentation3D
{
	...
	String generateScript()
	{
		return "import numpy as np" + "\n"
				...
				+ "hours, remainder = divmod(processing_time_ms, 3600000)" + "\n"
				+ "minutes, remainder = divmod(remainder, 60000)" + "\n"
				+ "seconds, milliseconds = divmod(remainder, 1000)" + "\n"
				+ "print(f\"Time for loading model: {int(hours):02}:{int(minutes):02}:{int(seconds):02}.{int(milliseconds):03}\")"
				+ "\n\n"
				+ "start_time = time.time() * 1000" + "\n"
				...
				+ "minutes, remainder = divmod(remainder, 60000)" + "\n"
				+ "seconds, milliseconds = divmod(remainder, 1000)" + "\n"
				+ "print(f\"Processing time for segmentation: {int(hours):02}:{int(minutes):02}:{int(seconds):02}.{int(milliseconds):03}\")"
				+ "\n\n"
