...
| Branch Sinuosity   | _idem_          | Computes the sinuosity of a spot during its life cycle (cf.[Sinuosity](https://en.wikipedia.org/wiki/Sinuosity)), i.e. how much the track represented by the branch is curved Values close to 1: almost straight movement. Values significantly higher than 1: winding or meandering movement. | $\frac{\text{track length}}{\text{direct distance}}$, i.e. \frac{\sum_{i=1}^{k} \| \mathbf{s_i} - \mathbf{s_{i+1}} \|}{\| \mathbf{s_i} - \mathbf{s_{k}} \|}, k=\text{number of spots of branch}, \mathbf{s_{i}} = i^{th}\text{spot vector} |
